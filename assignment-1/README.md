...
**บทบาทที่สมัคร:** AI Engineer

**ขอบเขตงาน:** Text → Transaction NER, การประเมินโมเดล และการแนะนำโมเดลสำหรับใช้งานจริง

โปรเจกต์นี้เป็นคำตอบสำหรับ Assignment 1 ของ Parnuan โดยรับข้อความการใช้เงินที่ผู้ใช้พิมพ์แล้วแยกออกมาเป็นรายการธุรกรรมที่มีข้อมูล 2 อย่าง
- `amount` จำนวนเงิน
...
โปรเจกต์นี้ทำเฉพาะชั้น `NER` หรือการดึงข้อมูลสำคัญออกจากข้อความ ยังไม่ทำการจัดหมวดหมู่ ไม่บันทึกฐานข้อมูล และไม่สร้าง timestamp
...
ผู้ใช้พิมพ์ข้อความเกี่ยวกับการใช้จ่าย เช่น `ซื้อข้าว 50 บาท และน้ำ 10 บาท` ระบบจะพยายามแยกออกมาเป็นรายการที่มี 2 ข้อมูลคือ `amount` (จำนวนเงิน) และ `detail` (รายละเอียดรายการ)

จุดสำคัญคือโมเดลภาษาไม่ได้เป็นคนตัดสินใจทั้งหมด ระบบมีโค้ด Python คอยกำหนดรูปแบบผลลัพธ์ ตรวจคำตอบ และปฏิเสธคำตอบที่ไม่น่าเชื่อถือ ถ้าตรวจไม่ได้ ระบบจะคืนรายการว่างแทนการเดาข้อมูลการเงินขึ้นมาเอง

### ลำดับการทำงานจริง

1. `CLI` หรือหน้า `local demo UI` รับข้อความจากผู้ใช้
2. `parser.py` ตรวจเบื้องต้นว่าข้อความไม่ว่างและมีความยาวไม่เกิน 4,000 ตัวอักษร
3. `prompts.py` สร้างคำสั่งให้โมเดล โดยกั้นข้อความผู้ใช้ไว้ใน `<input>` เพื่อไม่ให้ข้อความในใบเสร็จถูกตีความเป็นคำสั่งใหม่ของโมเดล
4. `client.py` ส่ง request ไปยังโมเดลที่เลือกผ่าน OpenRouter โดยอ่าน API key จาก environment variable ไม่เขียน key ลงในโค้ด
5. ระบบขอคำตอบเป็น JSON ตาม schema ที่กำหนด ถ้า provider ไม่รองรับ `JSON Schema` จะลดระดับไปใช้ `JSON object` แล้วตรวจซ้ำใน Python
6. `parser.py` อ่าน JSON และให้ Pydantic ตรวจว่าโครงสร้างถูกต้อง เช่น มีเฉพาะ `amount` กับ `detail` และไม่มี field แปลกปลอม
7. ระบบตรวจ `grounding` เพิ่มอีกชั้น โดยเช็กว่า amount และ detail ที่โมเดลตอบมีอยู่จริงในข้อความต้นฉบับ
8. ถ้าทุกอย่างผ่านจึงคืนรายการธุรกรรม ถ้าไม่ผ่านจะคืน `transactions: []` พร้อมสถานะ เช่น `invalid_model_output`, `rate_limited` หรือ `ungrounded_model_output`

### แต่ละไฟล์มีเหตุผลอะไร

| ไฟล์ | หน้าที่ | เหตุผลที่แยกไฟล์ |
|---|---|---|
| `schema.py` | กำหนด output contract ของระบบ | ทำให้ทุกช่องทางได้ผลลัพธ์รูปแบบเดียวกัน |
| `prompts.py` | เขียนกติกาให้โมเดล extract ข้อมูล | แก้ prompt ได้โดยไม่ปนกับโค้ด HTTP |
| `client.py` | ติดต่อ OpenRouter, อ่าน usage และ retry | เปลี่ยน provider หรือทดสอบ network ได้ง่าย |
| `parser.py` | คุม flow, parse, validate และ fallback | เป็น business logic หลักที่ใช้ร่วมกันทั้ง CLI และ UI |
| `evaluation.py` | รัน dataset และคำนวณ metrics | วัดผลซ้ำได้ ไม่ต้องตัดสินจากความรู้สึก |
| `web.py` | แสดงหน้าเว็บและรับส่ง request | UI เป็นชั้นแสดงผล ไม่ทำ logic ซ้ำในหน้าเว็บ |
| `tests/` | ทดสอบ input, output, error และ retry | ตรวจพฤติกรรมโดยไม่ต้องยิง API จริงทุกครั้ง |

### ทำไมไม่ให้โมเดลตอบแล้วนำไปใช้เลย

โมเดลภาษาเป็นระบบที่มีความน่าจะเป็น จึงอาจตอบ JSON ไม่ครบ ใส่ข้อความที่ไม่มีใน input หรือสร้างจำนวนเงินขึ้นมาเองได้ แม้ prompt จะสั่งไว้แล้วก็ตาม

ดังนั้นโปรเจกต์จึงแบ่งความรับผิดชอบเป็น 2 ส่วน:

- โมเดลช่วยอ่านภาษาธรรมชาติและเสนอรายการที่น่าจะพบ
- Python เป็นผู้ตรวจสัญญา (`schema`), ความสอดคล้องกับต้นฉบับ (`grounding`) และกำหนดว่าจะยอมรับหรือปฏิเสธคำตอบ

แนวคิดนี้ทำให้ระบบเหมาะกับข้อมูลการเงินมากขึ้น เพราะคำตอบที่ไม่ผ่านการตรวจจะไม่ถูกส่งต่อเป็นธุรกรรมที่ดูเหมือนถูกต้อง

### การจัดการปัญหาและ rate limit

ถ้า OpenRouter ตอบ HTTP 429 หรือ 503 client จะ retry แบบเว้นระยะเพิ่มขึ้น (`exponential backoff`) และอ่านค่า `Retry-After` ถ้า provider ส่งมาให้ เมื่อ retry ครบแล้วยังไม่สำเร็จ ระบบจะคืนรายการว่างพร้อมสถานะ ไม่ค้างและไม่ทำให้ผู้ใช้เห็นข้อมูลปลอม

การแยก `status` ออกจากผลลัพธ์ทำให้รู้ว่ารายการว่างเกิดจากอะไร เช่น ข้อความว่าง, โมเดลตอบผิดรูปแบบ, provider ล่ม หรือโดน rate limit ซึ่งสำคัญกว่าการเห็นแค่รายการว่างอย่างเดียว

### ทำไมต้องมีทั้ง CLI และ UI

`CLI` เหมาะกับการพัฒนาและทดสอบเร็ว เพราะเห็น input/output ตรง ๆ และใช้ใน benchmark ได้ ส่วน `local demo UI` เหมาะกับการสาธิตให้คนที่ไม่ใช่โปรแกรมเมอร์เห็นภาพการใช้งานจริง

ทั้งสองช่องทางเรียก `parser.py` ชุดเดียวกัน จึงไม่เกิดปัญหาว่าหน้าเว็บทำงานอย่างหนึ่ง แต่คำสั่งใน terminal ทำงานอีกอย่างหนึ่ง การมี UI จึงเป็นการเพิ่มวิธีนำเสนอ ไม่ใช่การเพิ่ม business logic ซ้ำ

### สิ่งที่ผล benchmark บอกได้และบอกไม่ได้

ผล benchmark เป็นหลักฐานจาก dataset และช่วงเวลาที่รันจริง ไม่ใช่คำรับรองว่าโมเดลจะถูกต้องทุกข้อความ โดยเฉพาะโมเดลฟรีที่อาจมี rate limit หรือผู้ให้บริการเปลี่ยนแปลงได้

เวลาอ่านผลควรดูอย่างน้อย 3 อย่างพร้อมกัน:

- `Amount F1` และ `Detail F1` ว่าแยกข้อมูลได้แม่นแค่ไหน
- `Exact Match` ว่าทั้งรายการตรงกับ label ครบหรือไม่
- `availability` และ failure taxonomy ว่าคำตอบที่หายไปเกิดจากคุณภาพโมเดลหรือระบบไม่พร้อมให้บริการ

ถ้าโมเดลถูก rate limit จำนวนมาก ค่า F1 ที่ต่ำอาจสะท้อนว่าเรียกใช้งานไม่สำเร็จ ไม่ได้แปลว่าโมเดลอ่านภาษาแย่เสมอไป จึงต้องรายงานคุณภาพและ availability แยกกัน

### ขอบเขตเรื่องข้อมูลส่วนตัว

โหมด offline ใช้ตรวจระบบโดยไม่ส่งข้อความออกจากเครื่อง แต่โหมด OpenRouter จะส่งข้อความไปยัง provider เพื่อให้โมเดลประมวลผล ดังนั้นการใช้ข้อมูลจริงต้องลบหรือปิดบังข้อมูลส่วนตัวก่อน และต้องใช้ API key ผ่าน environment variable เท่านั้น

**ทำไมเลือก Python?**  เพราะงานนี้เน้น data validation, การทดลองโมเดล และการทำ benchmark ซึ่ง Python มีเครื่องมือรองรับดี และแยก core logic ไปทดสอบได้ง่าย

**ถ้าโมเดลตอบผิด ระบบทำอย่างไร?**  ตรวจ JSON ด้วย Pydantic และตรวจซ้ำว่า amount/detail อยู่ใน input ถ้าไม่ผ่านจะไม่ส่งต่อคำตอบ แต่คืนรายการว่างพร้อมสถานะ

**ทำไมไม่เลือกโมเดลที่ใหญ่ที่สุด?**  เลือกจากผล benchmark และความพร้อมใช้งานจริง ไม่ใช่ดูจำนวนพารามิเตอร์อย่างเดียว ถ้าโมเดลใหญ่โดน rate limit มากก็อาจไม่เหมาะกับงานที่ต้องตอบสม่ำเสมอ

**ระบบนี้เป็น local จริงหรือไม่?**  หน้า UI และโค้ดรันในเครื่องได้ แต่ถ้าเลือกโหมด OpenRouter ข้อความจะถูกส่งไปประมวลผลที่ provider ส่วนโหมด offline เท่านั้นที่ไม่ส่งข้อความออกไป
...
ต้องตั้ง API key ผ่าน environment variable เท่านั้น
...
ถ้าเกิด `rate_limited` เวลาที่แสดงอาจรวมเวลาที่ระบบรอ retry ด้วย ไม่ใช่เวลาประมวลผลของโมเดลเพียงอย่างเดียว จึงไม่ควรนำ latency ของ request ที่ล้มเหลวไปเทียบตรง ๆ กับ request ที่ตอบสำเร็จ
...
สำหรับคนที่ไม่ถนัด terminal ให้เริ่มทดสอบหน้า demo ในโหมด Offline ก่อน โหมดนี้ต้องระบุ `--offline` เอง ไม่ใช่ค่าเริ่มต้นของคำสั่ง:
...
ถ้าต้องการทดสอบการเรียกโมเดลจริง ให้ตั้ง API key แล้วเปิด Online mode:
